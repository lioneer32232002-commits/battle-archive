...
//
// M-1 升級(art-upgrade-spec §2):航點間改用「以航點時間 t 為節點」的非均勻 Catmull-Rom
//   曲線插值(與 src/bastogne/engine/timeline.js 同一套實作),轉彎為平滑弧線、無瞬間轉頭。
//   時間參數沿用航點 t(非弧長參數化),對位置為「確定性映射」,確保拖曳跳轉時位置正確、不飛點。
//   起步/停止只對頭尾段各套一次緩動。
...
export function unitStateAt(unit, t) {
  ...
  let downAt = null; // 失效時刻(沉沒;與巴斯通共用寫法,一併認 destroyed)
  ...
  for (const c of unit.statusChanges ?? []) {
    ...
    if ((c.status === 'sunk' || c.status === 'destroyed') && downAt === null) downAt = c.t;
  }
