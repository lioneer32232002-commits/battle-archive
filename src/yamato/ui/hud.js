...
// 戰力血條值:'air' 艦載機存量、'flagship' 旗艦大和艦體完整度、'fleet' 加權存艦比例
function barValue(side, key, states) {
  const s = sides[side];
  if (key === 'air') {
    let total = 0;
    for (const id of s.unitIds) total += states.get(id)?.strength.aircraft ?? 0;
    return (total / (s.baseAircraft || 1)) * 100;
  }
  if (key === 'flagship') {
    const st = states.get(s.flagshipId);
    if (!st) return 0;
    if (st.status === 'sunk') return 0;
    return st.strength.hull ?? 0;
  }
  // 'fleet':依噸位加權的存艦比例(正常=1、燃燒/失能=0.4、沉沒=0)
  let have = 0;
  let max = 0;
  for (const id of s.unitIds) {
    ...
    const f = !st ? 1 : st.status === 'sunk' ? 0 : st.status === 'burning' ? 0.4 : 1;
    have += w * f;
  }
  return (have / (max || 1)) * 100;
}

export function createHUD(callbacks) {
  const root = document.getElementById('hud');

  // 手機常時顯示的迷你血條:依各陣營 bars 設定動態生成
  const miniRow = (side) => {
    const s = sides[side];
    const cells = s.bars
      .map(
        (b, i) => `
        <span class="hpm-cell">
          <i class="hpm-ico" title="${b.label}">${b.icon}</i>
          <span class="hpm-track"><span class="hpm-ghost" id="mini-hp-bar${i}-ghost-${side}"></span><span class="hpm-fill" id="mini-hp-bar${i}-${side}"></span></span>
          <span class="hpm-val" id="mini-hp-bar${i}-val-${side}">100%</span>
        </span>`
      )
      .join('');
    return `<div class="hpm-row ${side}"><span class="hpm-side"><i class="dot"></i>${side === 'red' ? '日軍' : '美軍'}</span>${cells}</div>`;
  };
  ...
  root.innerHTML = `
    <div id="topbar">
      <div class="tb-left">
        ...
        <span class="battle-title">天號作戰 <small>Operation Ten-Gō</small></span>
      </div>
      <div class="tb-instruments">
        ...
        <div class="tb-clock-frame">
          <span class="tbc-tag" aria-hidden="true">ZONE TIME · 1945</span>
    ...
    <div id="figure-card" class="hidden">
      <div class="figure-box">
        ...
        <div class="fig-result"><span class="rl">此役戰果</span><span id="fig-battle"></span></div>
        <div class="fig-result"><span class="rl">生涯結局</span><span id="fig-career"></span></div>
      </div>
    </div>
  `;
  ...
  function buildSummary() {
    ...
    const rows = outcome.rows
      ...
      .join('');
    // 排版規則(桌面與手機一致):陣營名稱獨立一行,括號內的軍種/番號降為下一行的副標,整段不在中途斷行。
  ...
  function setBar(side, idx, value) {
    const p = Math.max(0, Math.min(100, Math.round(value)));
    for (const prefix of ['hp', 'mini-hp']) {
      const fill = document.getElementById(`${prefix}-bar${idx}-${side}`);
      const ghost = document.getElementById(`${prefix}-bar${idx}-ghost-${side}`);
      const val = document.getElementById(`${prefix}-bar${idx}-val-${side}`);
  ...
  function buildPanel(side) {
    ...
    const initTotal = s.totalKind === 'aircraft' ? s.baseAircraft : `${s.unitIds.length} / ${s.unitIds.length} 艘`;
    el.innerHTML = `
      <h2>${s.name}</h2>
      <div class="hp-bars">
        ${s.bars
          .map(
            (b, i) => `
        <div class="hp-row">
          <span class="hp-label">${b.label}</span>
          <div class="hp-track"><div class="hp-ghost" id="hp-bar${i}-ghost-${side}"></div><div class="hp-fill" id="hp-bar${i}-${side}"></div></div>
          <span class="hp-val" id="hp-bar${i}-val-${side}">100%</span>
        </div>`
          )
          .join('')}
      </div>
      ...
      <div class="strength">
        <div class="strength-title">艦艇編成 / 航空戰力</div>
        ${s.unitIds
          .map((id) => {
            const u = units.find((x) => x.id === id);
            const nameHtml =
            ...
            return `<div class="unit-row${capId ? ' clickable' : ''}" id="row-${id}"${fig}>
              ...
              <span class="u-count" id="count-${id}">${u.strength.aircraft ?? u.classLabel ?? ''}</span>
            </div>`;
          })
          .join('')}
        <div class="unit-row total"><span class="u-name"><span class="zh">${s.totalLabel}</span></span><span class="u-count" id="total-${side}">${initTotal}</span></div>
      </div>
    `;
  }
    ...
    updatePanels(states) {
      for (const side of ['red', 'blue']) {
        ...
        for (const id of s.unitIds) {
          ...
          if (!st) continue;
          const u = units.find((x) => x.id === id);
          aircraftTotal += st.strength.aircraft ?? 0;
          if (st.status !== 'sunk') shipsAlive++;
          const countEl = document.getElementById(`count-${id}`);
          if (countEl) countEl.textContent = st.strength.aircraft ?? u?.classLabel ?? '';
        ...
        if (totalEl) {
          totalEl.textContent =
            s.totalKind === 'aircraft' ? aircraftTotal : `${shipsAlive} / ${s.unitIds.length} 艘`;
        }
        s.bars.forEach((b, i) => setBar(side, i, barValue(side, b.key, states)));
      }
    },
    showEvent(e) {
