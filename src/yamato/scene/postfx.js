// 後製特效(桌機限定,P-3):EffectComposer → RenderPass ＋ UnrealBloomPass(海戰:太陽反光、曳光彈、
//   大爆炸泛光;threshold 0.8(比陸戰低)讓海面 glitter 也能微微發亮)＋ OutputPass(色調映射＋輸出色彩空間)
//   ＋ 自寫 ShaderPass(暗角 vignette ＋ 輕微膠片顆粒)。
// 手機不建 composer,由 main.js 以同一個 renderFrame() 分流直接 renderer.render。
//
// ⚠ OutputPass 不可省(2026-09-12 第二輪整合實測):three 在「渲染到 render target」時會把材質的
//   tonemapping 與 colorspace 兩個 chunk 關掉,交給最後一關處理。少了這一關,桌機等於整條管線
//   既沒 ACES 也沒 sRGB 編碼、toneMappingExposure 完全無效 → 比手機暗一大截。
//   第一輪把調色盤整組往上拉一階就是在補這個洞,補上 OutputPass 之後那些數值全部要調回來。
...
export function createComposer(renderer, scene, camera) {
  ...
  const bloom = new UnrealBloomPass(new THREE.Vector2(size.x, size.y), 0.26, 0.62, 0.8); // strength/radius/threshold
