...
export function createYamato(spec) {
  ...
  const hull = makeHull(L, W, hullH, grey);
  g.add(hull);

  // 主甲板(深木色,略高於船舷)
  const deck = new THREE.Mesh(
    new THREE.BoxGeometry(W * 0.92, 1.2, L * 0.96),
    new THREE.MeshLambertMaterial({ color: 0x6b6256 })
  );
  deck.position.y = hullH + 0.6;
  g.add(deck);
  const deckY = hullH + 1.2;

  const turretMat = new THREE.MeshLambertMaterial({ color: 0x5b6068 });
  const barrelMat = new THREE.MeshLambertMaterial({ color: 0x44474d });

  // 46cm 三聯裝主炮塔:艦艏 2 座(背負式)、艦艉 1 座
  function mainTurret(z, aim, lift = 0) {
    const t = new THREE.Group();
    const base = new THREE.Mesh(new THREE.CylinderGeometry(W * 0.26, W * 0.3, 2.6, 10), turretMat);
    t.add(base);
    const box = new THREE.Mesh(new THREE.BoxGeometry(W * 0.42, 2.2, L * 0.075), turretMat);
    box.position.y = 1.6;
    t.add(box);
    for (let b = -1; b <= 1; b++) {
      const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, L * 0.16, 6), barrelMat);
      barrel.rotation.x = -Math.PI / 2;
      barrel.position.set(b * W * 0.12, 1.9, -L * 0.09 * aim);
      t.add(barrel);
    }
    t.position.set(0, deckY + lift, z);
    g.add(t);
  }
  ...
  const towerMat = new THREE.MeshLambertMaterial({ color: 0x70757d });
  const tiers = [
    [W * 0.5, 4, L * 0.12, 4],
    [W * 0.38, 4, L * 0.09, 8],
  ...
  for (const [w, h, d, y] of tiers) {
    const tier = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), towerMat);
    tier.position.set(0, deckY + y, -L * 0.05);
    g.add(tier);
  }
  const top = new THREE.Mesh(new THREE.CylinderGeometry(W * 0.07, W * 0.1, 2.2, 8), towerMat);
  top.position.set(0, deckY + 18, -L * 0.05);
  g.add(top);

  // 單一大型後傾煙囪
  const stack = new THREE.Mesh(
    new THREE.CylinderGeometry(W * 0.16, W * 0.2, 7, 12),
    new THREE.MeshLambertMaterial({ color: 0x35383d })
  );
  stack.position.set(0, deckY + 5, L * 0.07);
  stack.rotation.x = 0.18;
  g.add(stack);

  // 後桅
  const mast = new THREE.Mesh(
    new THREE.CylinderGeometry(0.22, 0.22, 12, 5),
    new THREE.MeshLambertMaterial({ color: 0x2b2e33 })
  );
  mast.position.set(0, deckY + 8, L * 0.17);
  g.add(mast);
  ...
  const wake = makeWake(L * 1.55, W * 2.2);
  g.add(wake);
  g.userData.wake = wake;
  return g;
}
