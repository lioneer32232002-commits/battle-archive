...
export function createHUD(callbacks) {
  ...
  root.innerHTML = `
    <div id="topbar">
      <div class="tb-left">
        ...
        <span class="battle-title">巴斯通 <small>Bastogne · 1944</small></span>
      </div>
      <div class="tb-instruments">
        ...
        <div class="tb-clock-frame">
          <span class="tbc-tag" aria-hidden="true">1944 · 阿登</span>
    ...
    <div id="legend">
      ...
      <span><i class="dot blue"></i>美軍 101 空降師</span>
      <span class="hint">藍線：交叉火網 · 紅線：德軍攻擊軸 · 藍環：殺戮區</span>
    ...
    <div id="intro">
      <div class="intro-box">
        <p class="intro-kicker">戰史檔案館 · 3D 戰術解析 · 突出部之役</p>
        <h1>巴斯通</h1>
        <p class="intro-sub">Bastogne · 1944/12/18–26 · 阿登 · 圍城</p>
        <p class="intro-desc">
          1944 年 12 月，101 空降師死守阿登的七路樞紐巴斯通：E 連在傑克森林的散兵坑裡，
          承受樹頂空爆與零度嚴寒；四面合圍，麥考利夫對德軍勸降只回一字「呸！」；
          天氣放晴後空投續命，終於由巴頓的第 4 裝甲師打通走廊解圍。
        </p>
        <button id="btn-start" class="hud-btn start">▶ 進入戰場</button>
        <p class="intro-src">資料依據：U.S. Army CMH（Cole《The Ardennes》、Marshall《Bastogne》）、army.mil、National WWII Museum、CSI 巴斯通參謀研究、Ambrose《Band of Brothers》；界線考據見內文（斷腿、陣亡、佛伊皆屬 1945 年 1 月，不屬本役）</p>
      </div>
    </div>
    ...
    <div id="figure-card" class="hidden">
      <div class="figure-box">
        ...
        <div class="fig-result"><span class="rl">巴斯通戰績</span><span id="fig-battle"></span></div>
  ...
  function buildSummary() {
    ...
    const achieve = outcome.achievement
      ? `<div class="sum-achieve"><span class="sa-tag">守城戰果</span>${outcome.achievement}</div>` : '';
  ...
  function rosterHtml(side) {
    ...
    const order = ['指揮官', '守線', '醫護', '解圍', '考據還原', '德軍'];
