// 環境：天空、地面、低空晨霧、雲層、星空、光照 — 依戰役時刻變化（拂曉前 → 日出）
// 十字路口為荷蘭島區的秋日陰霾：冷灰色調、低垂濃雲、貼地的圩田晨霧。
import * as THREE from 'three';

const PALETTES = {
  night: { top: 0x05080f, horizon: 0x10161f, sun: 0x5d6f8a, sunInt: 0.26, amb: 0.34, ground: 0x1a2016, fog: 0x0c1016 },
  dawn:  { top: 0x33414f, horizon: 0xb6a890, sun: 0xe8d9b8, sunInt: 0.72, amb: 0.60, ground: 0x3e4a2c, fog: 0x8a8f86 },
  day:   { top: 0x6d7d8c, horizon: 0xb9c2c4, sun: 0xdfe2dc, sunInt: 0.98, amb: 0.92, ground: 0x55633a, fog: 0xb3bcb8 },
};
...
export function createEnvironment(scene) {
  ...
  for (let i = 0; i < 70; i++) {
    const c = new THREE.Sprite(
      new THREE.SpriteMaterial({ map: cloudTex, color: 0x9aa3a0, transparent: true, opacity: 0.28 + rand() * 0.22, depthWrite: false })
    );
  ...
  for (let i = 0; i < 22; i++) {
    ...
    m.userData.baseOp = 0.22 + rm() * 0.18;
    mist.add(m);
  }
  scene.add(mist);
