...
// flyTo:一次性緩動飛到定點;follow:持續跟隨移動中的單位(如溫特斯突擊隊登堤東掃)。
//
// M-4 升級(鏡頭手感;實作同 src/bastogne/camera/director.js):
//   1. 手持噪聲 — follow 期間於最終 position/target 疊加微小偽 Perlin 偏移(數個不同頻率 sin 疊加),
//      幅度約跟拍距離的 0.3%;自由模式不加。
//   2. 衝擊震動 — shake(intensity, duration):衰減式隨機偏移,barrage／destroy／assault 觸發。
//   3. 跟拍前置量(look-ahead) — follow 目標點沿單位行進方向前移約跟拍距離的 8%,
//      畫面重心落在部隊「要去的地方」(上刺刀衝鋒段特別有感)。
