  ...
  if (models.length) {
    ...
    for (const e of models) {
      ...
      try {
        ...
        const triFlag = e.triBudget && s.triangles > e.triBudget ? ` ⚠ >${e.triBudget}` : '';
        ...
        console.log('  ' + e.id.padEnd(22) + fmt(st.size).padStart(10) + s.triangles.toLocaleString().padStart(10) +
          String(s.materials).padStart(5) + String(s.textures).padStart(5) + '  ' + sizes + flag + triFlag + alphaNote);
